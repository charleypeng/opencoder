...
const DirectoryPickerDialog: Component<DirectoryPickerDialogProps> = (props) => {
  ...
  /** Adds the browsed directory as the working directory: DesktopShell
   *  reacts to the store change by rebuilding the per-directory SSE stream
   *  and re-syncing, which loads the directory's sessions. */
  function add(): void {
    setCurrent(props.serverId, dir());
    pushRecentProject(props.serverId, dir());
    props.onClose();
  }
