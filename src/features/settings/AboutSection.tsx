// About settings section (TASK-M9-04): the app name and version
// (getAppVersion), the ACTIVE server's version and status (connection
// store health snapshot), the MIT license and the project links (GitHub /
// documentation / LICENSE), plus a copyright line. Outside Tauri the
// version readouts show an em dash (the facade resolves null).
...
const AboutSection: Component<AboutSectionProps> = (props) => {
  ...
  return (
    <div data-testid="about-section" class="flex min-h-0 flex-1 flex-col">
      <div class="shrink-0 border-b border-bg-sunken px-4 py-3">
        <h2 class="text-sm font-semibold">{t("settings:about")}</h2>
        <p class="text-xs text-fg-secondary">{t("settings:aboutHint")}</p>
      </div>
      <div class="min-h-0 flex-1 overflow-y-auto p-4">
        <div class="flex items-center justify-between gap-3 border-b border-bg-sunken py-3">
          ...
          <span data-testid="about-version" class="shrink-0 font-code text-xs text-fg-secondary">
            {version() ?? "—"}
          </span>
        </div>
        <div class="flex items-center justify-between gap-3 border-b border-bg-sunken py-3">
          ...
          <span
            data-testid="about-server-version"
        ...
        <p data-testid="about-copyright" class="py-3 text-xs text-fg-faint">
