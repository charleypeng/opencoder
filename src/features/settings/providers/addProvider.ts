...
// merging `provider.<id>` (a ProviderConfig: name? + options?) into the
// global or project config; enabled_providers/disabled_providers only gate
// enablement and are not touched here. This module owns that shape and the
// provider-id validation so the dialog and its L1 tests share one source
// of truth.
...
/** Builds the Config PATCH that registers the provider: `provider.<id>`
 *  with the trimmed display name and options.baseURL/apiKey; empty fields
 *  are omitted (a bare id yields `{ provider: { [id]: {} } }`, which the
 *  server treats as an enabled provider with its built-in endpoint). */
export function buildProviderPatch(id: string, input: AddProviderInput): ConfigPatch {
  const name = input.name?.trim() ?? "";
  const baseURL = input.baseURL?.trim() ?? "";
  const apiKey = input.apiKey?.trim() ?? "";
  const config: { name?: string; options?: { baseURL?: string; apiKey?: string } } = {};
